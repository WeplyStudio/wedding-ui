    ...
    <div className="flex justify-center gap-4 md:gap-8">
      {timeUnits.map(({ value, label }) => (
        <div key={label} className="flex flex-col items-center p-2 rounded-lg">
          <span className="text-4xl md:text-6xl font-headline font-bold">{value || 0}</span>
          <span className="text-sm md:text-base font-body uppercase tracking-wider">{label}</span>
        </div>
      ))}
    </div>
  );
